...
def get_words(characters: list[list[str]], origin: tuple[int, int], size: tuple[int, int]):
    # from given origin
    # can xmas be up
    x, y = origin
    ...
    if x - 3 >= 0:
        word_up = (
            characters[x][y] + characters[x-1][y] + characters[x-2][y]
            + characters[x-3][y]
        )
    # can xmas be down
    if x + 3 < x_limit:
    ...
    if x - 3 >= 0 and y - 3 >= 0:
        left_up = (
            characters[x][y] + characters[x-1][y-1]
            + characters[x-2][y-2] + characters[x-3][y-3]
        )
    ...
    return (
        word_left, word_right, word_up, word_down,
        left_up, left_down, right_up, right_down
    )
...
def is_xmas(characters: list[list[str]], origin: tuple[int, int]):
    x, y = origin
    left_top_right_down = characters[x-1][y-1] + characters[x][y] + characters[x+1][y+1]
    right_top_left_bottom = characters[x-1][y+1] + characters[x][y] + characters[x+1][y-1]
    if (
        (left_top_right_down == "MAS" or left_top_right_down[::-1] == "MAS")
        and
